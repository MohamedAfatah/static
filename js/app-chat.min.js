...
// async function* generateMessage(history, bot_message) {
//     for (const character of bot_message) {
//       history[history.length - 1][1] += character;
...
function appendMessage(img, side, text) {
    const msgHTML = `
        <div class="chat ${side}">
            ...
            <div class="chat-body">
                <div class="chat-text">
                    <p>${text.replace(/(https?:\/\/[^\s]+)/g, '<a target="_blank" href="$1">$1</a>')}</p>
                </div>
            </div>
...
function botResponse() {
    ...
    // if (sessionId === null) {
    //     sessionId = generateSessionId(); // function to generate session ID
